...
const UsuariosTipoList = ({ tipo, titulo, descripcion }) => {
  ...
  const filteredMessage = useMemo(() => {
    if (loading) return "";
    if (!usuarios || usuarios.length === 0) return t("sin_usuarios", "No se encontraron usuarios.");
    return t("mostrando_usuarios", {
      count: usuarios.length,
      total: pagination.total || usuarios.length,
    });
  }, [loading, usuarios, pagination.total, t]);

  return (
    <div className="admin-usuarios-page">
      <div className="admin-usuarios-header">
        <div>
          <h1>{titulo || t("gestion_usuarios", "Gestión de Usuarios")}</h1>
          <p>{descripcion || t("gestion_usuarios_desc", "Administra cuentas, estados y verificaciones.")}</p>
        </div>
      ...
      <div className="admin-usuarios-filters">
        ...
        <div className="filter-item">
          ...
          <select value={filters.tipo} disabled>
