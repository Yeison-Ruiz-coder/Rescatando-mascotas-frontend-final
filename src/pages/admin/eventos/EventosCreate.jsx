    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (error.response?.data?.errors) {
                ...
                alert('Error al crear el evento global');
            }
        } finally {
            setLoading(false);
        }
        ...
        <div className="eventos-form-container">
            <div className="form-card">
                <div className="form-card-header admin-header">
                    <h2>Crear Evento Global</h2>
                    <p>Este evento será visible para todos los usuarios del sistema</p>
                </div>
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="form-row">
                            <div className="form-group">
                                ...
                                <input type="text" name="Nombre_evento" className={`form-control ${errors.Nombre_evento ? 'is-invalid' : ''}`} value={formData.Nombre_evento} onChange={handleChange} placeholder="Ej: Gran Feria Nacional" />
                                {errors.Nombre_evento && <div className="invalid-feedback">{errors.Nombre_evento}</div>}
                            </div>
                            <div className="form-group">
                                <label><MapPin size={16} /> Lugar del Evento *</label>
                                <input type="text" name="Lugar_evento" className={`form-control ${errors.Lugar_evento ? 'is-invalid' : ''}`} value={formData.Lugar_evento} onChange={handleChange} placeholder="Ej: Centro de Convenciones" />
                        ...
                        <div className="form-actions">
                            <button type="button" onClick={() => navigate('/admin/eventos')} className="btn-secondary" disabled={loading}>Cancelar</button>
                            <button type="submit" className="btn-primary" disabled={loading}>{loading ? 'Creando...' : 'Crear Evento Global'}</button>
                        </div>
                    </form>
                </div>
            </div>
